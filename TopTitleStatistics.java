...
public class TopTitleStatistics extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        Path tmpPath = new Path("hdfs://localhost:9000/mp2/tmp");
...
//        FileOutputFormat.setOutputPath(jobA, tmpPath);
        FileInputFormat.setInputPaths(jobA, new Path("hdfs://localhost:9000/mp2/titles"));
        FileOutputFormat.setOutputPath(jobA, tmpPath);
        ...
        Path outputPath = new Path("hdfs://localhost:9000/B-output");
         outputPath.getFileSystem(confb).delete(outputPath, true);
...
//            this.delimiters = readHDFSFile(delimitersPath, conf);
            ...
            this.stopWords = Arrays.asList(readHDFSFile("/mp2/misc/stopwords.txt", conf).split("\n"));
            this.delimiters = readHDFSFile("/mp2/misc/delimiters.txt", conf);
        }
    ...
    public static class TopTitlesStatMap extends Mapper<Text, Text, NullWritable, TextArrayWritable> {
        Integer N;
        // TODO
        private TreeSet<Pair<Integer, String>> TopTitlesMap = new TreeSet<Pair<Integer, String>>();
        @Override
        protected void setup(Context context) throws IOException,InterruptedException {
            Configuration conf = context.getConfiguration();
        ...
        public void map(Text key, Text value, Context context) throws IOException, InterruptedException {
            // TODO
        	 Integer count = Integer.parseInt(value.toString());
	       	 ...
	       	 TopTitlesMap.add(new Pair<Integer, String>(count,
	       	word));
	       	 if (TopTitlesMap.size() > N) {
	       		 TopTitlesMap.remove(TopTitlesMap.first());
       		}
        }
        ...
        protected void cleanup(Context context) throws IOException, InterruptedException {
            // TODO
        	for (Pair<Integer, String> item : TopTitlesMap) {
        ...
        // TODO
            for (TextArrayWritable val: values) {
       		...
       		Integer.parseInt(pair[1].toString());
       		 TopTitlesMap.add(new Pair<Integer,
       		String>(count, word));
	       		 if (TopTitlesMap.size() > N) {
